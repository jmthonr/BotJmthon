...
from jmthon.database.admin import *
...
async def is_register_admin(chat, user):
    if isinstance(chat, (types.InputPeerChannel, types.InputChannel)):
        return isinstance(
            (
                await jmthon(functions.channels.GetParticipantRequest(chat, user))
            ).participant,
...
async def get_user_sender_id(user, event):
    ...
    try:
        user_obj = await jmthon.get_entity(user)
...
async def get_user_from_event(event):
    ...
    if event.reply_to_msg_id:
        ...
        user_obj = await jmthon.get_entity(previous_message.sender_id)
    else:
        ...
        if event.message.entities is not None:
            ...
            if isinstance(probable_user_mention_entity, MessageEntityMentionName):
                ...
                user_obj = await jmthon.get_entity(user_id)
                return user_obj
        try:
            user_obj = await jmthon.get_entity(user)
        except (TypeError, ValueError) as err:
...
def find_instance(items, class_or_tuple):
    for item in items:
        if isinstance(item, class_or_tuple):
            return item
    return None
@jmthon.on(events.NewMessage(incoming=True, pattern="^رفع مشرف ?(.*)"))
async def promote(promt):
    ...
    if promt.is_group:
      if not promt.sender_id == OWNER_ID:
        if not (await is_admin(promt.chat_id, promt.sender_id) or (not await is_admin(promt.chat_id, promt.sender_id) and await is_register_admin(promt.input_chat, promt.sender_id))):
           return #هنا بمكان ادمن مالك
    else:
        return
    ...
    try:
        await jmthon(EditAdminRequest(promt.chat_id, user.id, new_rights, title))
...
async def demote(dmod):
    ...
    try:
        await jmthon(EditAdminRequest(dmod.chat_id, user.id, newrights, "Admin"))
...
async def ban(bon):
    ...
    try:
        await jmthon(EditBannedRequest(bon.chat_id, user.id, BANNED_RIGHTS))
        if not reason:
...
async def unban(bon):
    ...
    try:
        await jmthon(EditBannedRequest(bon.chat_id, user.id, UNBAN_RIGHTS))
...
async def kick(bon):
    ...
    try:
        await jmthon.kick_participant(bon.chat_id, user.id)
...
async def spider(spdr):
    ...
    try:
        await jmthon(EditBannedRequest(spdr.chat_id, user.id, MUTE_RIGHTS))
...
@register(pattern="^مسح المحظورين$")
async def _(event):
    ...
    p = 0
    async for i in jmthon.iter_participants(
        event.chat_id, filter=ChannelParticipantsKicked, aggressive=True
    ):
        rights = ChatBannedRights(until_date=0, view_messages=False)
        try:
            await jmthon(functions.channels.EditBannedRequest(event.chat_id, i, rights))
...
async def pin(msg):
    ...
    is_silent = True
    try:
        await jmthon(UpdatePinnedMessageRequest(msg.to_id, to_pin, is_silent))
...
async def get_admin(show):
    ...
    try:
        async for user in jmthon.iter_participants(
...
async def set_group_photo(gpic):
    ...
    if photo:
        try:
            await jmthon(EditPhotoRequest(gpic.chat_id, await tbot.upload_file(photo)))
...
async def get_users(show):
    ...
    if show.is_group:
        if not await is_register_admin(show.input_chat, show.sender_id):
            return
    info = await jmthon.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
    mentions = "Users in {}: \n".format(title)
    async for user in jmthon.iter_participants(show.chat_id):
        if not user.deleted:
...
async def rm_deletedacc(show):
    ...
    async for user in jmthon.iter_participants(show.chat_id):
        if user.deleted:
            try:
                await jmthon(EditBannedRequest(show.chat_id, user.id, BANNED_RIGHTS))
                ...
                del_u -= 1
                del_a += 1
            await jmthon(EditBannedRequest(show.chat_id, user.id, UNBAN_RIGHTS))
            del_u += 1
...
@register(pattern=r"^وضع وصف ([\s\S]*)")
...
@tbot.on(events.NewMessage(pattern="^حظر مؤقت (.*)"))
async def ban(bon):
 try:
    if not bon.is_group:
        # print("1")
    ...
    if cid.isnumeric():
        cid = int(cid)
    entity = await jmthon.get_input_entity(cid)
    try:
...
Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks."""
    ...
    NEW_RIGHTS = ChatBannedRights(
                 ...
                 embed_links=True)
    try:
     await jmthon(EditBannedRequest(bon.chat_id, r_sender_id, NEW_RIGHTS))
